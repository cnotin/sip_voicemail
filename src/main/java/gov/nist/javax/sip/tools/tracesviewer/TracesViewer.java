...
package gov.nist.javax.sip.tools.tracesviewer;
...
public class TracesViewer extends javax.swing.JFrame {
	...
	/**
	 * Call this constructor when you want to construct a visualizer from traces
	 * that you have already read.
	 * 
	 * @param titles
	 *            is an array containing the titles for the traces.
	 * @param traces
	 *            is an array containing an array of XML Formatted traces.
	 * 
	 */
	public TracesViewer(Hashtable traces, String logName,
			String logDescription, String auxInfo) {
	...
	public TracesViewer(String title, String aRmiHost, String aRmiPort,
			String aStackId) {
		super(title);
		try {
			/*
			...
			URL url = TracesViewer.class.getResource("back.gif");
			// System.out.println("url:"+url.toString());
			backgroundImage = toolkit.getImage(url);
			url = TracesViewer.class.getResource("comp.gif");
			actorsImage = toolkit.getImage(url);
			url = TracesViewer.class.getResource("faces.jpg");
			facesImage = toolkit.getImage(url);
			url = TracesViewer.class.getResource("nistBanner.jpg");
	...
	public TracesViewer(String title, String aRmiHost, String aRmiPort,
			String aStackId, String back, String faces, String actors,
			String aLogoNist) {
		super(title);
		try {
			Toolkit toolkit = Toolkit.getDefaultToolkit();
	...
	// Constructor for the Application only!!!!!
	public TracesViewer(String logFile, Hashtable traces, String logName,
			String logDescription, String auxInfo, String title, String back,
			String faces, String actors, String aLogoNist) {
		super(title);
		...
		try {
			 ...
			 * facesImage=toolkit.getImage(faces); this.logoNist=logoNist;
			 */
		} catch (Exception e) {
			backgroundImage = null;
			actorsImage = null;
	 ...
	 */
	public TracesViewer(String title, String logFile, String back,
			String faces, String actors, String logoNist) {
		super(title);
		this.logFile = logFile;
		listenerTracesViewer = new ListenerTracesViewer(this);
	...
	public TracesViewer(String port, String logFile, Hashtable traces,
			String logName, String logDescription, String auxInfo,
			String title, String back, String faces, String actors,
			String aLogoNist) {
		super(title);
		this.logFile = logFile;
	...
	public TracesSessions refreshTracesSessions() {
		...
		try {
			...
			if (trace != null && !trace.equals("")) {
				...
				Enumeration elements = traces.elements();
				while (elements.hasMoreElements()) {
					MessageLogList mll = (MessageLogList) elements
							.nextElement();
					TracesSession ts = new TracesSession(mll);
	...
	private void initComponents() {
		...
		animationMenuItem
				.setToolTipText("Animation of the entire selected session");
		...
		displayAllSessionsMenu
				.setToolTipText("Display all the retrieved sessions in a separate windows");
		...
		PercentLayoutConstraint firstPanelConstraint = new PercentLayoutConstraint(
				30, 0, 70, 100);
		...
		TracesSession tracesSession = (TracesSession) tracesSessions
				.firstElement();
		...
		if (name.equals("No available session, refresh")) {
			tracesCanvas = new TracesCanvas(tracesSession,
					messageContentTextArea, "unknown", this);
		} else if (logDescription == null || logDescription.trim().equals("")) {
			tracesCanvas = new TracesCanvas(tracesSession,
					messageContentTextArea, origin, this);
		} else {
			// System.out.println("logDesc44:"+logDescription);
			tracesCanvas = new TracesCanvas(tracesSession,
					messageContentTextArea, logDescription, this);
		}
		...
		PercentLayoutConstraint secondPanelConstraint = new PercentLayoutConstraint(
				0, 0, 30, 100);
		...
		if (!standaloneViewer) {
			...
			scriptLabel
					.setToolTipText("Display the content of the selected script");
			...
			scriptButton
					.setToolTipText("Get the script controlling the current session");
			 ...
			 * refreshButton.addActionListener(new ActionListener() { public
		...
		messageContentButton
				.setToolTipText("Display all the content of the current SIP message");
		// Alignment of the text
		...
		messageContentButton.setBorder(BorderFactory
	...
	public static void usage() {
		...
		System.out.println("*************************************");
		System.out
				.println("Use this tool to view the signaling trace as a sequence Diagram");
	...
	public static void main(String args[]) {
		try {
			if (args.length == 0) {
				...
				System.out
						.println("Everything is Ok ... Launching the Traces viewer");
				TracesViewer tracesViewer = new TracesViewer("Traces viewer",
						"127.0.0.1", "1099", "127.0.0.1:5060");
				...
				new TracesViewer(fileName, traces, parser.logName,
						parser.logDescription, parser.auxInfo, "traces viewer",
						"back.gif", "faces.jpg", "comp.gif", "nistBanner.jpg")
						.show();
				...
				new TracesViewer(port, fileName, traces, parser.logName,
						parser.logDescription, parser.auxInfo,
						"traces viewer daemon", "back.gif", "faces.jpg",
						"comp.gif", "nistBanner.jpg").show();
				...
				if (stackId == null) {
					System.out.println("Stack Id (name) not specified Bailing!"
				...
				if (launcher)
					tracesViewer = new TracesViewer("Traces viewer", rmiHost,
							rmiPort, stackId, back, faces, actors, logoNist);
				else
					tracesViewer = new TracesViewer("Traces viewer", rmiHost,
							rmiPort, stackId);
				System.out
